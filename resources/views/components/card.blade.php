<div class="grid grid-cols-4 gap-x-[30px]">
    @foreach($user->items->where('id', '!=', $item->id) as $card)
        <div class="bg-[#343444] rounded-[20px] h-[511px] w-[330px] p-[20px] flex flex-col gap-y-[21px]">
            <img src="{{asset('uploads')."/".$card->img_item}}" class="image-preview bg-[#7A798A] w-[290px] h-[290px] rounded-[20px] relative group block object-cover">
                ...
                <div class="flex gap-x-[95px] items-center">
                    <div class=>
                        ...
                        <div class="flex gap-x-[7px] items-center">
                            ...
                            <h4 class="text-[#8A8AA0] font-normal text-[13px] leading-[20px]">= ${{$card->price*12.246}}</h4>
                        </div>
                    </div>
